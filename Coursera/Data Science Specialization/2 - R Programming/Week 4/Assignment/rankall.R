...
rankall <- function(truc,num = "best") {
  ...
  outcome <- outcome[order(outcome$State,outcome$value,outcome$Hospital.Name),]
  ...
  soutcome <- split(outcome,outcome$State)
  data.frame(t(sapply(soutcome,"[",num,,drop=FALSE)))
}
